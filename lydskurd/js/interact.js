...
LS.interact = (function () {
   ...
   * @returns {{clip: object, mode: 'move'|'trimStart'|'trimEnd'}|null}
   */
  function targetAt(p) {
  ...
  function moveClip(p, deltaSec, noSnap) {
    ...
    const wanted = Math.max(0, drag.origTimeStart + deltaSec);
    clip.timeStart = Math.max(0, snapStart(wanted, clip.srcLen, clip.id, noSnap));

    // Sporet under peikaren overtek klippet.
    const track = LS.render.trackAtY(p.y);
    if (track) clip.trackId = track.id;
  }
  ...
  function trimStart(deltaSec, noSnap) {
    ...
    // Klippet skal heller ikkje kunne skyvast ut i negativ tid.
    const limited = Math.max(d, -drag.origTimeStart);
  ...
  function trimEnd(deltaSec, noSnap) {
    ...
    const maxLen = sourceLength(clip) - drag.origSrcStart;
    clip.srcLen = LS.util.clamp(drag.origSrcLen + deltaSec, MIN_LEN, maxLen);
  }
  ...
  function end(e) {
    ...
    if (changed) {
      ...
      // Berre eit klikk — då er utvalet heile endringa, og angre-steget unødig.
      LS.state.undo();
